...
xs = (P({
        0: P().oval(sq),
        1: P(sq).layer(3).stack(0),
        2: P(sq).layer(4).stack(0),
        3: P(qc().r90(-1), qc()).stack(0),
        4: P(qc().rotate(-90), fl, fl, sq, qc()).stack(0),
        5: P(qc().rotate(-90), fl, fl, sq, sq, qc()).stack(0),
        6: qc(),
        7: P(sq, qc()).stack(0),
        8: P(sq).layer(2).extend([fl, qc()]).stack(0),
        9: P(sq).layer(3).extend([fl, qc()]).stack(0),
    })
...
glyphs = (P(dict(
    ...
    b=P(xs[2], xs[4].flipx().ch(under)),
    c=P(xs[4].ch(under), xs[7].align(xs[1],"N")),
    d=P(xs[4].ch(under), xs[2]),
    e=P(xs[4].ch(under), xs[7].r90(2).align(xs[8],"N")),
    f=P(xs[9].flipy(), P(xs[6].align(xs[9],"N"), xs[7].align(xs[1], "N"))),
    g=P(xs[4].ch(under), xs[9].flipx().align(xs[1],"N")),
    h=P(xs[2], xs[8].r90(2)),
    i=P(P(xs[1], xs[0].t(0, xs[1].h+20))),
    j=P(P(xs[9].flipx().align(xs[1],"N"), xs[0].t(0, xs[1].h+20))),
    k=P(xs[2], P(xs[6].flipx().align(xs[1],"N").t(0, 10), xs[7].r90(2))),
    l=P(xs[9]),
    m=P(xs[1], xs[1], xs[8].r90(2)),
    n=P(xs[1], xs[8].r90(2)),
    o=P(xs[4].ch(under)).layer(1, lambda p: p.flipx()),
    p=P(xs[2].align(xs[1], "N"), xs[4].flipx().ch(under)),
    q=P(xs[4].ch(under), xs[2].align(xs[1], "N")),
    ...
    u=P(xs[8].ch(under)).layer(1, lambda p: p.flipx()),
    v=P(xs[8].r90(2), xs[8].flipx().ch(under)),
    w=P(xs[8].ch(under), xs[1], xs[8].flipx().ch(under)),
    x=P(xs[6].align(xs[4], "N"), xs[7].flipy()).layer(1, lambda p: p.flipx()),
    y=P(xs[8].ch(under), xs[9].flipx().align(xs[1], "N")),
    ))
    .map(lambda p: p.spread(30))
    .mapv(lambda p: p.difference(p.copy().outline(5))))
